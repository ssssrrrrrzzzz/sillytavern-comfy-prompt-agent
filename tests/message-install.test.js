...
test('installer lays out both halves and enables server plugins with backup', () => {
    ...
    assert.ok(fs.existsSync(path.join(root, 'plugins/comfy-prompt-agent/releases/0.3.1/server-plugin/lib/jobs.js')));
    assert.ok(fs.existsSync(path.join(root, 'plugins/comfy-prompt-agent/releases/0.3.1/shared/context.js')));
    assert.ok(fs.existsSync(path.join(root, 'plugins/comfy-prompt-agent/releases/0.3.1/shared/version.js')));
    assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'plugins/comfy-prompt-agent/active-version.json'), 'utf8')).version, '0.3.1');
