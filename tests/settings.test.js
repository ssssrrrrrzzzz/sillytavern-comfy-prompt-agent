...
test('all numeric mode and ComfyUI settings clamp and boolean settings survive partial updates', () => {
    const mode = normalizeModeSettings({ historyTurns: 999, maxInputTokens: 1, maxOutputTokens: 999999, timeoutSeconds: 0 }, defaultConfig.modes[3], true);
    ...
    assert.equal(mode.maxSteps, defaultConfig.modes[3].maxSteps);
    assert.equal(mode.includePersona, defaultConfig.modes[3].includePersona);
