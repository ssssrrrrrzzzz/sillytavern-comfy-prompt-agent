...
test('runtime prompts are applied after preset snapshots and overrides cannot touch negatives', () => {
    ...
    assert.throws(() => applyWorkflowPreset(template, preset, 'x', { 2: { text: 'LLM negative' } }), /not allowed/);
});

test('preset can keep a fixed seed and an allowlisted seed wins over randomization', () => {
