...
- 将 SillyTavern 代理返回的嵌套 JSON 网络错误整理为简短、可读的中文提示。
- 网络重试不迁移、不改写模式 2 系统提示词或其他用户配置。

## 0.5.6

- Anima 模式允许用户自定义提示词要求的多行角色块，逐行规范标签并保留换行；格式修复不再强制单行输出。

## 0.5.5

- 拒绝 LLM/格式修复返回的网络、API 或服务错误文本；Anima 输出至少需要两个英文逗号分隔标签，失败时明确报错而不提交错误出图。

## 0.5.4

- Anima 模式拒绝中日韩文字标签，并使用设置界面可见的模式 2 提示词执行一次英文格式修复，避免中文标签直接送入 ComfyUI。

## 0.5.3

- 将 Anima Skill 的适用生成规则整理为模式 2 默认系统提示词，并在 SillyTavern 设置中完整显示、编辑和保存。
- 删除首次生成和格式修复路径中不可见的 Anima 附加提示词；LLM 的生成指令以设置文本框为唯一来源。
- 仅迁移历代内置默认值，用户自定义的模式 2 提示词保持不变。
