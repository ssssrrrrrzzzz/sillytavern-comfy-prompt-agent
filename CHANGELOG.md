...
- 后台任务在非当前 Swipe 完成时，成功提示会明确图片所在 Swipe 与当前 Swipe，不再造成“成功但没图”的误解。
- 切回已经完成出图的 Swipe 时主动刷新媒体包装与插件画廊，立即恢复该 Swipe 的图片和正向 Prompt。
- 等待 SillyTavern 真正保存新 Swipe 后才提交出图任务；删除或替换目标 Swipe 后不再重建位置或串写图片。
- 新生成的 Swipe 不再继承上一 Swipe 的 completed/job/image 状态，并以当前正文的 trigger hash 决定是否幂等跳过。
- 模式 1 保留原始标签用于重试，但聊天界面改为“正在出图 / 重试 / 图片”状态占位，不再展示整段出图标签。
- 自动提交会等待 SillyTavern 与翻译扩展完成当前 Swipe 落盘；估算和提交入口不再自行创建 Swipe。
- 等待期间切换聊天后，返回原聊天会恢复尚未提交的模式 1 出图状态，不会永久卡在“正在出图”。
- 模式 2 会显式读取同一消息较早 Swipe 的正向 Prompt 作为一致性参考，不再依赖继承旧任务状态。

## 0.5.9

- Profile 输出上限支持到 131,072 token，`64,000` 不再被截断成 `32,768`。
- 保存 LLM Profile 时自动把该输出上限同步到模式 2，避免模式 2 仍停留在 1,024。
- 旧配置中“Profile 高于 1,024、模式 2 仍为默认 1,024”的情况会自动迁移。
